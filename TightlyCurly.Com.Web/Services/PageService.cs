...
public class PageService : IPageService
{
    ...
    public Page GetPageById(int pageId)
    {
        ...
        if (!_cacheHelper.CacheEnabled || !_cacheHelper.IsInCache($"page{pageId}"))
        {
            ...
            {
                _cacheHelper.InsertCache($"page{pageId}", page, _configurationHelper.DefaultCacheExpiration, TimeSpan.Zero);
            }
        }
        else
        {
    ...
    public Page GetPageByName(string name)
    {
        ...
        if (!_cacheHelper.CacheEnabled || !_cacheHelper.IsInCache($"page{name}"))
        {
            ...
            if (_cacheHelper.CacheEnabled)
            {
                _cacheHelper.InsertCache($"page{name}", page, _configurationHelper.DefaultCacheExpiration, TimeSpan.Zero);
            }
        }
        else
        {
